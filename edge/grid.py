...
'''
 ...
 LastEditors  : yurui
 LastEditTime : 2022-01-04 14:03:25
...
def graymap(name_val, val, path):
    cv2.imshow(name_val, val)
    cv2.imwrite(path + name_val + ".jpg", val)
    cv2.waitKey(-1)
...
if __name__ == '__main__':
    ...
    grid_size_x, grid_size_y = int(
        (xmax-xmin)/grid_x), int((ymax-ymin)/grid_y)  # [120,80]

    num_of_points = np.zeros((grid_size_x, grid_size_y)
                             )  # [120,80] 用于记录每个网格中点的数量
    points_of_grids = np.zeros(
        (grid_size_x, grid_size_y, max_num_of_point, num_of_points_features))  # 用于记录每个grid中点的坐标
    ...
    #!--------------------------高程
    #!可以直接使用points_of_grids，减少两次循环复杂度
    ...
    for i in range(grid_size_x):
        for j in range(grid_size_y):
            ...
            height_of_grids[i, j] = np.sum(
                points_of_grids[i, j, :num_point_in_grid, 2])/num_point_in_grid  # 单元格内部的平均高度
    ...
    for i in range(grid_size_x):
        for j in range(grid_size_y):
            ...
            if j != 0 and j != grid_size_y-1:
                d_value_of_height_y[i, j] = max(abs(
                    height_of_grids[i, j-1]-height_of_grids[i, j]), abs(height_of_grids[i, j]-height_of_grids[i, j+1]))
    ...
    for i in range(grid_size_x):
        for j in range(grid_size_y):
            ...
            if i != 0 and i != grid_size_x-1:
                d_value_of_height_x[i, j] = max(abs(
                    height_of_grids[i-1, j]-height_of_grids[i, j]), abs(height_of_grids[i, j]-height_of_grids[i+1, j]))
    ...
    # graymap("num_of_points", num_of_points, path)
    # graymap("d_value_of_height_y", d_value_of_height_y, path)
    # graymap("d_value_of_height_x", d_value_of_height_x, path)
    graymap("var*100", var*100, path)
